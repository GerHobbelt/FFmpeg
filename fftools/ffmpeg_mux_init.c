                            ...
                            enum VideoSyncMethod *vsync_method)
{
    ...
    AVStream *st;
    const char *frame_rate = NULL, *min_frame_rate = NULL, *max_frame_rate = NULL, *frame_aspect_ratio = NULL;
    int ret = 0;
    ...
    opt_match_per_stream_str(ost, &o->min_frame_rates, oc, st, &min_frame_rate);
    if (min_frame_rate && av_parse_video_rate(&ms->min_frame_rate, min_frame_rate) < 0) {
    ...
    if (min_frame_rate && max_frame_rate && av_q2d(ms->min_frame_rate) > av_q2d(ms->max_frame_rate)) {
    ...
    if (ost->enc) {
        ...
        *vsync_method = VSYNC_AUTO;
        ...
        if ((ms->frame_rate.num || ms->min_frame_rate.num || ms->max_frame_rate.num) &&
            !(*vsync_method == VSYNC_AUTO ||
