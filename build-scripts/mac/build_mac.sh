...
if [[ "$1" -eq 1 ]]; then
	...
	FLAGS=(--extra-cflags="-I${CONAN_ARM}/include/videoai -I${CONAN_ARM}/include $5" --extra-ldflags="-L${CONAN_ARM}/lib -headerpad_max_install_names $6" --extra-libs="${CONAN_ARM}/lib/libssl.a ${CONAN_ARM}/lib/libcrypto.a" --enable-tvai ${FLAGS[@]})
	XFLAGS=(--arch=x86_64 --extra-cflags="-arch x86_64 -I${CONAN_X64}/include/videoai -I${CONAN_X64}/include $5" --extra-ldflags="-arch x86_64 -L${CONAN_X64}/lib -headerpad_max_install_names $6" --extra-libs="${CONAN_X64}/lib/libssl.a ${CONAN_X64}/lib/libcrypto.a" --enable-shared --disable-static --enable-cross-compile --enable-tvai --enable-libvpx --enable-libaom --enable-libzimg --enable-openssl --disable-ffplay --disable-libxcb --disable-sdl2 --disable-xlib)
fi
